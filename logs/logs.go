		...
		case <-t.C:
			...
			for _, c := range l.collections {
				...
				for _, src := range srcs {
					...
					go l.runSrcToDest(&destinationTarget{src: src, dest: dest, backend: backend})
				}
			}
		case <-ctx.Done():
			return
...
func (l *LogAgent) runSrcToDest(destTarget *destinationTarget) {
	src := destTarget.src
	dest := destTarget.dest
