...
func TestNewPusher(t *testing.T) {
	var s svcMock
	p := NewPusher(Target{"G", "S"}, &s, time.Second, maxRetryTimeout, models.NewLogger("cloudwatchlogs", "test", ""), &sync.Mutex{}, false)
	if p.Service != &s {
...
func TestAddSingleEvent(t *testing.T) {
	...
	p := NewPusher(Target{"G", "S"}, &s, 1*time.Hour, maxRetryTimeout, models.NewLogger("cloudwatchlogs", "test", ""), &sync.Mutex{}, false)
...
func TestIgnoreOutOfTimeRangeEvent(t *testing.T) {
	...
	p := NewPusher(Target{"G", "S"}, &s, 10*time.Millisecond, maxRetryTimeout, models.NewLogger("cloudwatchlogs", "test", ""), &sync.Mutex{}, false)
