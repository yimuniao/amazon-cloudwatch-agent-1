...
func TestNewPusher(t *testing.T) {
	var s svcMock
	p := NewPusher(Target{"G", "S", false}, &s, time.Second, maxRetryTimeout, models.NewLogger("cloudwatchlogs", "test", ""), &sync.Mutex{})
	if p.Service != &s {
...
func TestAddSingleEvent(t *testing.T) {
	...
	p := NewPusher(Target{"G", "S", false}, &s, 1*time.Hour, maxRetryTimeout, models.NewLogger("cloudwatchlogs", "test", ""), &sync.Mutex{})
...
func TestIgnoreOutOfTimeRangeEvent(t *testing.T) {
	...
	p := NewPusher(Target{"G", "S", false}, &s, 10*time.Millisecond, maxRetryTimeout, models.NewLogger("cloudwatchlogs", "test", ""), &sync.Mutex{})
